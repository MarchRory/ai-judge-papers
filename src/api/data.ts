...
/* 考试详情相关API */
export function classScoreApi(data: Paging<{ examId: number }>) {
  return request.post<ListResponse<ClassScore>>(statisticsApi.classComp, data);
}

export function focusStuApi(data: Paging<{ examId: number; subject: number }>) {
  return request.post<ListResponse<FocusStu>>(statisticsApi.focusStu, data);
}

export function gradeListApi(data: { examId: number; caseRange: { min: number; max: number; name: string } }) {
  return request.post<ListResponse<Grade>>(statisticsApi.gradeList, data);
}

export function importantQustionsApi(data: Paging<{ examId: number }>) {
  return request.post<ListResponse<ChartQuestion>>(statisticsApi.questions, data);
}

export function stuProgressListApi(data: Paging<{ subjectId: number; examId: number }>) {
  return request.post<ListResponse<ProgressStu>>(statisticsApi.stuProgress, data);
}

export function stuExamRankList(data: Paging<{ examId: number }>) {
